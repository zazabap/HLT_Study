# Author: Shiwen An 
# Purpose: Calculate out the 35 25 trigger not passing 30 20
...
def tree_online_pt_i(tree):
    indices = set()
    for i in range(len(tree.TrigMatched_Taus_HLTptfl)) :
        if ( tree.TrigMatched_Taus_HLTptfl[i].Pt() >= 35 or tree.TrigMatched_Taus_HLTptfl[i].Pt() <= 30): continue
        for  j in range(len(tree.TrigMatched_Taus_HLTptfl)) :
            if (i==j): continue
            if ( tree.TrigMatched_Taus_HLTptfl[j].Pt() >= 25 or tree.TrigMatched_Taus_HLTptfl[j].Pt() <=20 ): continue
            indices.add(tree.TrigMatched_Taus_HLTptfl[i].Pt())
            indices.add(tree.TrigMatched_Taus_HLTptfl[j].Pt())
    return indices
